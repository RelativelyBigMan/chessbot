...
uniform_size = 10
plt.scatter(range(len(games)), durations,s=uniform_size)
a, b = np.polyfit(range(len(games)), durations, 1)
plt.plot(range(len(games)), a*range(len(games))+b,linewidth=3,color='black',label="increasing time to compute the moves")  
